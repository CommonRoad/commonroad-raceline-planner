...
from commonroad_raceline_planner.planner.ftm_planner.trajectory_planning.side_of_line import side_of_line
...
class SplineApproxLayer(BaseRacetrackLayer):
    ...
    def spline_approximation(
        ...
        tck_cl, t_glob_cl = splprep(
            x=[interpol_track.x_m, interpol_track.y_m],
            k=k_reg,
            s=s_reg,
            per=1)[:2]
        ...
        dists_cl: np.ndarray = np.zeros(original_track.num_points)  # contains (min) distances between input points and spline
        closest_point_cl: np.ndarray = np.zeros((original_track.num_points, 2))  # contains the closest points on the spline
        closest_t_glob_cl: np.ndarray = np.zeros(original_track.num_points)  # containts the t_glob values for closest points
        t_glob_guess_cl: np.ndarray = original_track.track_length_per_point / original_track.track_length  # start guess for the minimization
        ...
        w_tr_right_smoothed_cl = np.interp(np.linspace(0.0, 1.0, no_points_reg_cl), closest_t_glob_cl,
                                           w_tr_right_new_cl)
