...
def spline_approximation(interpolated_track: DtoRacetrack,
                         k_reg: int = 3,
                         s_reg: int = 10,
    ...
    # original track
    original_track = copy.deepcopy(interpolated_track)
    original_track.close_racetrack()

    # interpolate racetrack
    interpolated_track.close_racetrack()
    interpolated_track.linear_interpol_racetrack(interpol_stepsize=stepsize_prep)
    ...
    tck_cl, t_glob_cl = interpolate.splprep([interpolated_track[:, 0], interpolated_track[:, 1]],
                                            k=k_reg,
                                            s=s_reg,
                                            per=1)[:2]
